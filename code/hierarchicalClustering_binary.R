...
# Determine order for metabolites in heatmap based on heirarchical clustering
...
binDist <- dist(binData, method="binary")
binCluster <- hclust(d=binDist, method="average")
print(paste("Correlation between cophenetic distances and actual distances:", cor(binDist, cophenetic(binCluster))))
binDenData <- dendro_data(as.dendrogram(binCluster), type="rectangle")
metaboliteOrder <- label(binDenData)$label
heatmapData$metabolite <- factor(heatmapData$metabolite, levels=metaboliteOrder)

# Modify order metabolites determined by heirarchical clustering
...
# Transform data into binary and wide format to use for heirarchical clustering 
binData <- subset(heatmapData, select=-c(concentration_ppm, stError_ppm, stError_microM))
binData$concentration_microM[binData$concentration_microM>0] <- 1
binData$concentration_microM <- as.factor(binData$concentration_microM)
binData <- binData %>%
...
speciesList <- vector(mode="character", length=nrow(label(binDenData)))
cladeList <- vector(mode="numeric", length=nrow(label(binDenData)))
for (i in 1:nrow(label(binDenData))){
  denSpecies <- label(binDenData)$label[i]
...
denCladeData <- data.frame(x=1, y=1:nrow(label(binDenData)), speciesList, cladeList)
denCladeData$cladeList <- factor(denCladeData$cladeList, levels=c(1, 2, 3, 4, 5, 6))

# Create dendrogram
binDenPlot <- ggplot() +
  geom_segment(data=segment(binDenData), mapping=aes(x=x, y=y, xend=xend, yend=yend)) +
  geom_text(data=label(binDenData), mapping=aes(x=x, y=y, label=label, hjust=1), nudge_y=0.8) +
  coord_flip() +
...
speciesOrder <- label(binDenData)$label
...
# Combine dendrogram and heatmap into 1 figure
finalFigure <- plot_grid(binDenPlot, heatmap, cladeLabels, nrow=1, rel_widths=c(0.575, 0.4, 0.025))
print(finalFigure)