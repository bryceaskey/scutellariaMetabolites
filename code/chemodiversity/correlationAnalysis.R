...
freshData <- rbind(fresh, frozenKR, wrightii, suffrutescencs)
...
# Merge fresh and herbarium data into  a single data frame
...
# Adjust species order to match order in phylogenetic tree
speciesOrder <- cladeData$species
allData$species <- factor(allData$species, levels=speciesOrder)
allData <- allData[order(allData$species), ]
allData$species <- droplevels(allData$species)

# Adjust flavonoid order to match order in biosynthetic pathway
...
pcaPlot <- ggplot() +
  ...
  scale_x_continuous(breaks=c(-1.0, -0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 1.0)) +
  scale_y_continuous(breaks=c(-1.0, -0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 1.0)) +
  theme_classic() +
  theme(legend.position="none",
...
varPlot <- ggplot(data=pca_vars, mapping=aes(x=Dim.1, y=Dim.2, label=variable, fill=metaboliteClass)) +
  ...
  xlab(paste("PC1 (", pc1_expl, "%)", sep="")) +
  ylab(paste("PC2 (", pc2_expl, "%)", sep="")) +
  scale_x_continuous(breaks=c(-2.5, -2.0, -1.5, -1.0, -0.5, 0, 0.5, 1.0, 1.5, 2.0, 2.5)) +
  scale_y_continuous(breaks=c(-2.5, -2.0, -1.5, -1.0, -0.5, 0, 0.5, 1.0, 1.5, 2.0, 2.5)) +
  theme_classic() +
