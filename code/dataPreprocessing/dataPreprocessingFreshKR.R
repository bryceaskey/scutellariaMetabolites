...
rawData <- read.csv("C:/Users/Bryce/Research/scutellariaMetabolites/data/hplc/raw_data/20191214_freshKR.csv")

# Define functions for interpreting injection names
abbrevNames <- data.frame(
  abbrev=c("IC", "BT", "DE", "IN", "PA", "ST"),
  fullName=c("indica var. coccinea", "barbata", "dependens", "insignis", "pekinensis var. alpina", "strigillosa")
)
...
abbrevOrgans <- data.frame(
  abbrev=c("L", "S", "R", "F"),
  fullOrgan=c("leaves", "stems", "roots", "flowers")
)
...
write.csv(allData, file="C:/Users/Bryce/Research/scutellariaMetabolites/data/hplc/preprocessed/20200117_frozenKR.csv", row.names=FALSE)
