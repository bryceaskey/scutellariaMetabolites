#!/bin/bash
#SBATCH --job-name=wgs-assemUnmapped-HAV       # Job name
...
#SBATCH --output=wgs-assemUnmapped-HAV_%j.log  # Standard output and error log

pwd; hostname; date

module load  intel/2018.1.163 boost/1.71.0 openmpi/3.1.0 sparsehash/2.0.3 abyss/2.1.0

echo "Assembling unmapped havanensis reads into contigs"

sp=HAV
reads=/ufrc/lee/braskey/Data/WGS/bwa/unmapped/
contigs=/ufrc/lee/braskey/Data/WGS/bwa/contigs/

abyss-pe name=${contigs}${sp}/${sp}_unmapped k=64 in="${reads}${sp}_unmapped_1.fq ${reads}${sp}_unmapped_2.fq"